...
export const AGENT_PITFALLS: readonly { title: string; body: string }[] = [
  ...
  },
  {
    ...
    body: 'The scope is 1024 samples of whatever is sounding at that instant, so a note the human finished a moment ago reads as silence — and taking that for "they played nothing, I heard nothing" is the false negative. The live output is kept for about four seconds: `analyze_audio({"source":"recent"})` measures that rolling buffer, and `capture_audio` measures it too and can first BLOCK until a human actually plays (`waitForSignal: true`), which is the answer to "the human is about to play something, tell me what you hear". Measured either way — a tool result here is a JSON value with no audio content block, so no tool returns samples you could listen to. Offline renders never reach the live graph and so never appear in it; for a sound nobody is playing, `render_audio` still renders the whole note with no user gesture.'
  },
  {
