...
- `render_audio` — renders a note sequence and returns metrics. `mode: 'offline'` (the default) renders deterministically through `OfflineAudioContext`, faster than realtime and without a user gesture; `mode: 'realtime'` captures the live graph. `format` selects `'metrics'` (default) or `'url'`, a page-local blob URL of the rendered WAV that a human can click to hear it — a few dozen bytes, and the only playback route this transport has. The samples themselves are never returned: a WebMCP tool result is a JSON value with no audio content block, so a Base64 WAV would be tens of thousands of tokens no model could play.
...
- `capture_audio` — the only tool that waits: `waitForSignal: true` blocks up to `maxWaitSeconds` for the live output to rise above the onset floor, then measures the `captureSeconds` that follow, which is what `analyze_audio({ source: 'recent' })` cannot do. Left off, it measures the tail of the same rolling buffer and returns at once. A window nobody played into comes back as `silent: true` with a note saying so. Offline renders bypass the live graph, so `render_audio` output never appears here.
...
- **`analyze_audio({ source: 'scope' })` reads a 1024-sample buffer (~21 ms), so
  ...
  for about four seconds: `analyze_audio({ source: 'recent' })` measures the
  last few seconds of it, and `capture_audio` measures the same rolling buffer
  and can first block until someone plays (`waitForSignal: true`). One of those
  is the answer to "the human just played something, did you hear it?".
