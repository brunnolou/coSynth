// Factory presets: the built-in patches the preset browser lists under
// "Factory". They live in shared/ rather than ui/ so that anything without a
// DOM - the WebMCP tools, a worker, a test - can list and load them. Before
// this module they were private to the preset dropdown, so `list_presets` and
// `load_preset` could only see localStorage user presets, and an agent had to
// read the DOM to even learn the names.
//
// Keep this module free of browser globals so it stays importable from an
// AudioWorklet or a Web Worker; `factory-presets.test.ts` greps the source for
// them and runs this import under the node environment to hold that line.

import type { PresetData } from '../audio/engine'
import { paramDef, valueToNorm } from './params'
import { defaultLfoShape, FX_IDS } from './messages'
import { PRESET_VERSION, validatePresetData } from './preset-store'

/**
 * A factory patch as authored: a name, the parameters it departs from the init
 * patch on, and its mod matrix. Everything else (`version`, `lfoShapes`,
 * `fxOrder`) is filled in by `getFactoryPreset`, so a preset only spells out
 * what makes it distinctive. `engine.loadPreset` takes a `Partial<PresetData>`
 * and resets to defaults first, which is why the partial form is loadable too.
 */
export interface FactoryPreset extends Partial<PresetData> {
  name: string
  /** Normalized 0..1, as stored - see `P()`. */
  params: Record<string, number>
  mods?: PresetData['mods']
}

/**
 * Convenience: author factory presets in raw units, store normalized.
 *
 * The direction matters. Presets persist normalized 0..1 values, but a cutoff
 * reads as 300 (Hz) and a decay as 0.4 (s) here; inverting this call would
 * silently detune every factory sound rather than fail.
 */
...
/** Names in dropdown order. `Init` is first and is the default selection. */
export function listFactoryPresetNames(): string[] {
  return FACTORY_PRESETS.map(preset => preset.name)
}

/**
 * The named factory patch as a complete, validated preset - the same shape
 * `loadPreset` returns for a user preset, so `engine.loadPreset()` takes it
 * directly. Returns null when no factory preset carries that name; a factory
 * preset that failed validation would throw, and `factory-presets.test.ts`
 * validates all of them so that cannot reach a caller.
 *
 * A fresh object every call: the result is handed to the engine, and the
 * authored entries in `FACTORY_PRESETS` must not become shared mutable state.
 */
export function getFactoryPreset(name: string): PresetData | null {
  const preset = FACTORY_PRESETS.find(entry => entry.name === name)
  if (!preset) return null
  return validatePresetData({
