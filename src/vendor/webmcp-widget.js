/**
 * Vendored from @jason.today/webmcp@0.1.13 (src/webmcp.js), MIT licensed.
 * https://github.com/jasonjmcghee/webmcp
 *
 * Why vendored rather than a dependency or a CDN <script>:
 *  - The published build/webmcp.js is an IIFE whose only export path is a
 *    `typeof module !== 'undefined'` guard, so loaded as a browser <script>
 *    it exposes nothing at all - no global, no window property.
 *  - The npm package pulls ~109 transitive packages for its Node bridge CLI,
 *    none of which the browser widget needs.
 * This copy is the dependency-free widget source with two edits, so Vite can
 * bundle it into a lazy chunk: its trailing `module.exports` guard is removed
 * (under Vite's ESM transform `module` is defined and the namespace object is
 * getter-only, so the assignment throws), and a real ESM export is appended.
 * Update by re-copying src/webmcp.js from a newer release and redoing both.
 */
